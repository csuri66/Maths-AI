...
def graph_to_pyg_data_reverse(G, group_size,verbose=False):
...
def graph_to_pyg_data_offset(G, group_size,verbose=False):
    ...
    for node in set1:
        prefs_for_gale = list(range(group_size,group_size*2))
        minimum = min(prefs_for_gale)
        maximum = max(prefs_for_gale)

        prefs_for_gale = [minimum if x == maximum else x + 1 for x in prefs_for_gale]
